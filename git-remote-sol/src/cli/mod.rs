...
pub struct CLI<'a> {
    ...
    stdin: &'a mut dyn std::io::BufRead,
...
impl<'a> CLI<'a> {
    ...
    fn handle_command(&mut self, command: &str) -> Result<(), Box<dyn std::error::Error>> {
        match command {
            "capabilities" => {
                writeln!(self.stdout, "{}", self.remote_helper.capabilities().join(","))?;
            }
            _ => {
